...
| **S2** | Remove the Nicki tail | Deleted `sheep-subtask`, `sheep-execute`, `subtask-maker/`, `execute-plan/`, `validation/`; routing is `start → spec → gherkin → review → acceptance → sync → archive → sync → integrate → close` (`gherkin → review` is a placeholder until S4 adds `red`); `fix` step and the review fix-loop replaced by the settled jump-to-gherkin/spec model; `current-task/subtasks/` no longer scaffolded; permissions keys ↔ agent files a bijection. Rule file and fixtures updated too (write-domain table had missed them). `errors-recording`'s `validation_errors` schema untouched. Audited: suite green, graph terminates at `done`, no assertion weakened (`routing_next_step` 15→13 = 3 removed + 1 added). Commit `5478d3a`. Follow-up by the architect: README's remaining subtask/execute lines, which the prompt had wrongly fenced off. |
| | **Spec output path is caller-owned** | `routing.json` said `specs/<slug>.json`, `spec-maker` defaulted to `current-task/specs/<slug>.json`. Now routing names `current-task/specs/` and the skill carries no default — the caller packs the path, per the existing ownership rule. Found during S2, fixed alongside it. |
