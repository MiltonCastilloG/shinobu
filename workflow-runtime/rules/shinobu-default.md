...
Sheep cannot reach the user; you and Shinobu can. When a sheep return (or Shinobu's relay of one) carries `open_questions`, put each question to the user with `AskQuestion` — offer the entry's `options` when present — then continue: re-spawn the same sheep with the answers (ad-hoc), or pass the answers back to Shinobu so she re-spawns (pipeline). For `spec` / `subtasks`, keep the pause path in the prompt so the sheep resumes. Never answer for the user.
...
- **Document sheep** (`sheep-gherkin`, `sheep-spec`, `sheep-subtask`) — an output path. Use the user's path when they name one, otherwise a sensible file under `docs/adhoc/`. For `sheep-gherkin`, also pack the spec path. For `sheep-spec` and `sheep-subtask`, also pack a pause path (e.g. under `docs/adhoc/`) so the sheep can stop mid-run without losing exploration.
...
- **Operational sheep** (`sheep-execute`, `sheep-review`) — the working directory. If unclear, ask the user with `AskQuestion`; do not guess.
- **Git sheep** (`sheep-sync`, `sheep-integrate`) — get an explicit yes naming the side effect before spawning.

**When a sheep returns `open_questions`:** it has stopped, not failed. Put each question to the user with `AskQuestion` (offer the entry's `options` when present), then re-spawn the **same** sheep with the answers. For `spec` / `subtasks`, include the pause path so it resumes instead of re-exploring. Never answer for the user.
